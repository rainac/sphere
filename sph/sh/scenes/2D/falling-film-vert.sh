...
export SPH_PARAM_INIT_A0_P1="0                       $(($filmBottom + $dx))         0"
export SPH_PARAM_INIT_A0_P2="$(($omegaRight * 0.8))  $(($filmBottom + $filmWidth))  0"
...
if [[ "$boundaries" == "guide" || "$boundaries" == "both" ]]; then
    python $SPH_ROOT/sh/genline-2d.py -l $(($filmLeft + $filmWidth -$SPH_PARAM_DX)) -r $omegaRight -d 0.46 -u $(( $omegaTop - 0.03 )) -x $dx_border >> $SPH_PARAM_BOUNDARY_FILE
    python $SPH_ROOT/sh/genline-2d.py -r $(($filmLeft + $filmWidth -$SPH_PARAM_DX)) -l $omegaRight -d 0.46 -u 0.46 -x $dx_border >> $SPH_PARAM_BOUNDARY_FILE
