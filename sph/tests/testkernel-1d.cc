...
class TestKernel1D : public TestFixture {

  CPPUNIT_TEST_SUITE( TestKernel1D );
...
//     kernelNames.push_back("wendland3"); not available in 1D
  ...
  void nonZeroInsideReach() {
    for(size_t i = 0; i < kernelNames.size(); ++i) {
      double const r = H - (kernelNames[i].compare("johnson") == 0 ? 1e-9 : 1e-12);
...
//       std::cout << dist << " " << dnorm << " " << grad << "\n";
  ...
  void combinedFunction() {
    ...
    for(size_t i = 0; i < kernelNames.size(); ++i) {
      ...
      CPPUNIT_ASSERT_MESSAGE(kernelNames[i], (grad1 == grad2).min() == 1);
    }
  }
...
//           std::cout << k << " " << grad1[k] - grad2[k] << "\n";
  ...
  void checkIntegralIsOne() {
    ...
    double const dx = (3*H) / N;

    for(size_t i = 0; i < kernelNames.size(); ++i) {
      // integrate from 0 to 3*H ( is zero for > H )
      double sum = 0;
      for(size_t k = 0; k < N-1; ++k) {
        double const v1 = k * dx;
        double const v2 = (k+1) * dx;
        
        double const f1 = kernels[i]->w(v1);
        double const f2 = kernels[i]->w(v2);

        sum += dx * (f1 + f2) / 2;
      }

      // consider also negative direction
      sum *= 2;

      if (kernelNames[i] == "gauss") {
        CPPUNIT_ASSERT_DOUBLES_EQUAL_MESSAGE(kernelNames[i], 1.0, sum, 1e-3);
      } else if (kernelNames[i] == "lucy") {
        CPPUNIT_ASSERT_DOUBLES_EQUAL_MESSAGE(kernelNames[i], 1.0, sum, 1e-7);
      } else if (kernelNames[i] == "johnson") {
        CPPUNIT_ASSERT_DOUBLES_EQUAL_MESSAGE(kernelNames[i], 1.0, sum, 1e-6);
      } else {
...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION( TestKernel1D,
                                       coreSuiteName() );
